...
import { reqGet, reqCheck } from '@/api/userCenter';
...
export default {
  ...
  setup(props /* , context*/) {
    ...
    let passFlag = ref(''); // 是否通过的标识
    let backImgBase = ref(''); // 验证码背景图片
    let blockBackImgBase = ref(''); // 验证滑块的背景图片
    let backToken = ref(''); // 后端返回的唯一token值
    let startMoveTime = ref(''); // 移动开始的时间
    let endMovetime = ref(''); // 移动结束的时间
    let tipsBackColor = ref(''); // 提示词的背景颜色
    let tipWords = ref('');
    let text = ref('');
    let finishText = ref('');
    ...
    let top = ref(0);
    let left = ref(0);
    let moveBlockLeft = ref(undefined);
    let leftBarWidth = ref(undefined);
    // 移动中样式
    let moveBlockBackgroundColor = ref(undefined);
    let leftBarBorderColor = ref('#ddd');
    let iconColor = ref(undefined);
    let iconClass = ref('icon-right');
    let status = ref(false); // 鼠标状态
    let isEnd = ref(false); // 是够验证完成
