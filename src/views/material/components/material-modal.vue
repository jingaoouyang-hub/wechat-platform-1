<template>
  <a-modal v-model:visible="visible" title="添加素材" :width="600" @ok="handleOk" @cancel="handleCancel">
    <a-form layout="vertical" :model="formState" :rules="rules" ref="formRef" :labelCol="{ style: { width: '100px' } }">
      <a-form-item label="公众号链接" name="articlesUrl" required>
        <a-input v-model:value="formState.articlesUrl" placeholder="请输入公众号链接" />
      </a-form-item>
...
<script setup>
import { postMaterialAdd } from '@/api/wechat-manage/material';
...
const handleOk = () => {
  formRef.value.validate().then(() => {
    postMaterialAdd(formState).then(() => {
      visible.value = false;
